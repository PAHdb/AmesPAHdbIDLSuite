...
;
; DESCRIPTION:
;    Class to hold the Ames PAH geometry data
;
; CATEGORY:
;    GEOMETRY
;
...
;
; CREATION:
;    geo = OBJ_NEW('AmesPAHdbIDLSuite_Geometry')
;
; METHODS
;    PUBLIC:
;      PLOT (FUNCTION)
;      STRUCTURE (FUNCTION)
;      DIAGONALIZE (PROCEDURE)
;      MASS (FUNCTION)
;      INERTIA (FUNCTION)
;      RINGS (FUNCTION)
;      AREA (FUNCTION)
...
PRO AmesPAHdbIDLSuite_Geometry::Diagonalize,Full=Full,Equal=Equal
    ...
    FOR j = 0, self.nuids - 1 DO BEGIN
       ...
       ; Sort the Eigenvectors from smallest to large
       srt = SORT(D)
       I = I[srt, *]

       ; Project the molecule
       x = (*self.data)[select].x & y = (*self.data)[select].y & z = (*self.data)[select].z

       FOR l = 0, nselect - 1 DO BEGIN

          (*self.data)[select[l]].x = x[l] * I[0,0] + y[l] * I[0,1] + z[l] * I[0,2] 
          (*self.data)[select[l]].y = x[l] * I[1,0] + y[l] * I[1,1] + z[l] * I[1,2] 
          (*self.data)[select[l]].z = x[l] * I[2,0] + y[l] * I[2,1] + z[l] * I[2,2] 
       ENDFOR
    ENDFOR
END
